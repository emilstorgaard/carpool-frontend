...
export default function StartTrip() {
	...
	const startTrip = async (e: React.FormEvent) => {
		...
		try {
			await postTrip(userId, 80, isCarpool, new Date().toISOString(), new Date().toISOString())
            router.push('/trips')
		} catch (err) {
    ...
    return (
      <div className="page-container-narrow py-8">
          <div className="panel">
                    ...
                    <form className="space-y-5" onSubmit={startTrip}>
                        <div>
                        ...
                        <button type="submit" className="btn-success w-full">Start Trip</button>
                    </form>
          </div>
      </div>
    );
